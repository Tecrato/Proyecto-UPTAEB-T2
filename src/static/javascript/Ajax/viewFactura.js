                    ...
                    <div class="uk-flex uk-flex-between uk-flex-middle uk-background-secondary Target_factura" id=${t.id}>
                        <div class="uk-flex uk-flex-middle uk-margin-medium-right">
                            <div class="uk-margin-small-right">
                                <img src="src/static/images/logo_m.png" alt="" width="50px">
                            </div>
                            <div class="uk-flex uk-flex-column uk-flex-center">
                                ...
                                <h6 class="uk-margin-remove text_fact_info status_fact ${t.active == 0 ? "activeEmpty" : "uk-text-emphasis"}">${t.active == 1 ? "PAGADO" : "CREDITO"}</h6>
                            </div>
                        </div>
                        <div>
                            <div>  
...
let searchViewFact = document.querySelector(".search_viewFact");
searchViewFact.addEventListener("keyup", (e) => {
  let val = e.target.value;
  $.ajax({
    url: "api_search",
    type: "POST",
    data: { randomnautica: "ventas", like_nombre_cliente: val },
    success: function (response) {
      targetFacur(response);
    }
  })
})
let FORM_FACT_WIEW = document.querySelector(".FORM_DATE_WIEWFACT")
FORM_FACT_WIEW.addEventListener("submit", (e) => {
  e.preventDefault()
  let fecha_inicio = FORM_FACT_WIEW.firstElementChild.firstElementChild.lastElementChild.value
  let fecha_fin = FORM_FACT_WIEW.firstElementChild.lastElementChild.lastElementChild.value
  $.ajax({
    url: "api_search",
    type: "POST",
