<footer class="uk-light">
    <div class="uk-position-bottom-right uk-position-small uk-position-fixed ">
        <a href="#modal-details-product" uk-toggle>
            <span class="Bg-trash uk-icon-link" uk-icon="icon: trash; ratio: 3.5"></span>
        </a>
    </div>
</footer>

<script src="static/javaScript/hammer.min.js"></script>
<script src="static/javaScript/jquery.js"></script>
<script src="static/javaScript/funcionesSistema.js"></script>
<script src="static/javaScript/ajax.js"></script>
<script src="static/javascript/cosas.js" type="text/javascript"></script>

</body>
</html>